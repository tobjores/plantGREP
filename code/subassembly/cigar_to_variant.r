...
WT <- tibble(
  id = str_replace(WT.seqs[seq(1, length(WT.seqs), 2)], fixed('>'), ''),
...
get_mut <- function(id, cigar, start, seq) {
    ...
    mutations <- tibble(
      ...
      mutate(
        mutation = case_when(
          ...
          type == 'X' & bases == 1 ~ paste(pos.wt, str_sub(WT[WT$id == id, 'sequence'], start = pos.wt, end = pos.wt), '>', str_sub(seq, start = pos.mut, end = pos.mut), sep = ''),
...
subassembly <- read_tsv(infile) %>%
  ...
  mutate(
    ...
    variant = get_mut(id, cigar, start, sequence)
  ) %>%
  select(-cigar, -sequence) %>%
  select(barcode, id, start, stop, variant, everything())
