...
READBASENAME="eVal_subassembly"

REFSEQ="../../data/refseq/validation_sequences.fa"

READDIR="../../reads/subassembly/eVal"
...
pandaseq -N -f ${READDIR}/${READBASENAME}_insert_1.fastq.gz -r ${READDIR}/${READBASENAME}_insert_2.fastq.gz -d bfsrkm -G ${TMPDIR}/panda_logs/log_eVal_insert.txt.bz2 -w ${TMPDIR}/eVal_insert.fa
pandaseq -N -f ${READDIR}/${READBASENAME}_barcode_1.fastq.gz -r ${READDIR}/${READBASENAME}_barcode_2.fastq.gz -d bfsrkm -G ${TMPDIR}/panda_logs/log_eVal_barcode.txt.bz2 -w ${TMPDIR}/eVal_barcode.fa
...
awk -v tmpdir=${TMPDIR} -v OFS='\t' '{
    ...
    NAME=substr($1, 2);
    getline;
    if (length($1) == 18 && substr($1, 7, 1)substr($1, 13, 1) == "AG") print NAME, $0 > (tmpdir "/barcodes_pPSntF_eVal_35SprAG.tsv");
  }' ${TMPDIR}/eVal_barcode.fa

rm ${TMPDIR}/eVal_barcode.fa


### join barcodes and inserts ###
echo "joining barcodes and inserts ($(date +"%T"))"

LIB="pPSntF_eVal_35SprAG"
...
Rscript filter_subassembly_eVal.r ${TMPDIR}/subassembly_${LIB}.tsv ${CONTROLDIR}/barcodes_pPSntF_controls_35SprCC.txt.gz \
  > ${TMPDIR}/subassembly_eVal_filtered.tsv
